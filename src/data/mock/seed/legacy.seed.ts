 ...
 */
import type { AttributeSchema, FieldDef, ResultTemplate, SelectOption, ServiceType, TableColumn, TableField, TemplateDoc, TemplateElement } from '@/domain'
...
function tableSchema(p: LgProduct, rows: LgNatija[]): AttributeSchema {
  ...
  const preset = rows
    ...
    .map((r) => {
      ...
      for (const c of cols) o[c.key] = c.key === 'name' ? r.name : c.key === 'norma' ? (r.norma ?? '') : c.key === 'natija1' && isAntibiogram ? (r.name1 ?? '') : c.key === 'natija3' && isAntibiogram ? (r.norma1 ?? '') : ''
      return o
    })
...
export function buildLegacyCatalog(): { serviceTypes: ServiceType[]; schemas: AttributeSchema[]; templates: ResultTemplate[] } {
  ...
  for (const p of data.product) {
    ...
    if (rows.length >= 2) {
      ...
      else if (p.category_id === 2) { schemaId = 'sch_bak'; defaultTemplateId = 'tpl_generic' }
  ...
  return { serviceTypes, schemas, templates }
}
