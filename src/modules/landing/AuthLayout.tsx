...
export function AuthLayout({ children, aside }: { children: ReactNode; aside?: ReactNode }) {
  ...
  return (
    <div className="auth-shell relative min-h-dvh overflow-x-clip bg-bg">
      ...
      <main className={cn(
        'auth-shell-content mx-auto grid max-w-6xl gap-6 px-3 pb-16 pt-4 xs:px-5 sm:px-8 sm:pt-6 lg:items-start lg:gap-10 lg:pt-14 2xl:max-w-7xl 2xl:gap-16',
        aside ? 'lg:grid-cols-[440px_minmax(0,1fr)] 2xl:grid-cols-[480px_minmax(0,1fr)]' : 'justify-items-center',
      )}>
        <motion.div
          initial={reduce ? false : { opacity: 0, y: 14 }}
          animate={reduce ? undefined : { opacity: 1, y: 0 }}
          transition={{ duration: reduce ? 0.01 : 0.45, ease: [0.22, 1, 0.36, 1] }}
          className={cn('auth-panel w-full min-w-0 max-w-md justify-self-center rounded-[var(--radius-lg)] border border-line bg-bg-elevated p-4 shadow-2 xs:p-5 sm:p-7', aside ? 'lg:max-w-none lg:justify-self-start' : 'lg:max-w-[480px]')}
        >
          {children}
        </motion.div>
