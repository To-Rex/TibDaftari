...
export function ResultPreviewCard() {
  return (
    <div className="relative">
      ...
      <div className="rounded-[22px] border border-line bg-bg-elevated shadow-3">
        ...
        <div className="px-4 py-4 xs:px-5">
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-line">
            ...
            {rows.map((r, i) => (
              <motion.div key={r.name} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.09, duration: 0.4 }} className="grid grid-cols-[1.3fr_0.8fr_0.9fr] items-center gap-x-1 border-t border-line px-3 py-2.5 text-[13px]">
      ...
      {/* floating SMS bubble */}
      <motion.div initial={{ opacity: 0, y: 12, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 1.1, duration: 0.5, ease: [0.22, 1, 0.36, 1] }} className="absolute -bottom-12 left-0 flex w-[min(260px,100%)] items-start gap-3 rounded-2xl border border-line bg-bg-elevated p-3 shadow-3 xs:-left-3 xs:p-3.5 sm:-left-8 lg:-bottom-10">
