  ...
  return (
    <div className={cn('flex flex-wrap items-center gap-2', className)}>
      ...
      <AnimatePresence initial={false}>
        {value.preset === 'custom' && (
          <motion.div initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -6 }} transition={{ duration: 0.18 }} className="flex items-center gap-1.5">
            <Input type="date" value={value.range.from} max={value.range.to} onChange={(e) => onChange({ preset: 'custom', range: { ...value.range, from: e.target.value } })} className="h-8 w-[150px] px-2 text-[13px] tabular" />
            <span className="text-ink-3">–</span>
            <Input type="date" value={value.range.to} min={value.range.from} onChange={(e) => onChange({ preset: 'custom', range: { ...value.range, to: e.target.value } })} className="h-8 w-[150px] px-2 text-[13px] tabular" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
