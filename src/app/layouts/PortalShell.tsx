  ...
  return (
    <div className="min-h-dvh bg-bg">
      <header className="sticky top-0 z-30 border-b border-line bg-bg/80 backdrop-blur-md">
        <div className={cn('mx-auto flex h-14 sm:h-16 items-center gap-2 sm:gap-4 px-3 xs:px-4 sm:px-6', PORTAL_WIDTH)}>
          ...
          <nav className="ml-1 lg:ml-6 hidden md:flex min-w-0 items-center gap-1">
            {items.map((it) => (
              <NavLink key={it.to} to={it.to} end={it.end} title={it.label} className={({ isActive }) => cn('relative flex h-9 items-center gap-2 rounded-full px-3 lg:px-3.5 text-[14px] font-medium transition-colors', isActive ? 'text-brand-ink' : 'text-ink-2 hover:text-ink hover:bg-surface-2')}>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 flex border-t border-line bg-bg-elevated/95 backdrop-blur md:hidden pb-[env(safe-area-inset-bottom)]">
        {items.map((it) => (
          <NavLink key={it.to} to={it.to} end={it.end} title={it.label} className={({ isActive }) => cn('flex min-h-[52px] min-w-0 flex-1 flex-col items-center justify-center gap-1 px-1 py-2 text-[11px] font-medium', isActive ? 'text-brand' : 'text-ink-3')}>
